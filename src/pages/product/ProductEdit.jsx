...
import {
  INSERT_PRODUCT_MEDIA_ONE,
  UPDATE_PRODUCT_BY_PK,
} from "../../graphql/mutation/product.jsx";
...
import {
  DELETE_PRODUCT_MEDIA_BY_PK,
  UPDATE_PRODUCT_MEDIA_BY_PK,
} from "../../graphql/mutation/productMedia.jsx";
import useUploadFile from "../../utils/utils.jsx";
import { LoadingAnimation } from "../../components/LoadingAnimate.jsx";
import FormConstantSelect from "../../components/form/FormConstantSelect.jsx";
...
const ProductEdit = () => {
  ...
  // api call
  const { loading, data: product } = useQuery(PRODUCTS_BY_PK, {
    variables: { id },
  });
  const { data: productCategory } = useQuery(PRODUCT_SUB_CATEGORY_ALL);

  const [updateProduct] = useMutation(UPDATE_PRODUCT_BY_PK, {
    refetchQueries: [
      { query: PRODUCTS },
      { query: PRODUCTS_BY_PK, variables: { id } },
    ],
  });
  const [insertProductMedia] = useMutation(INSERT_PRODUCT_MEDIA_ONE, {
    refetchQueries: [{ query: PRODUCTS_BY_PK, variables: { id } }],
  });
  const [updateProductMedia] = useMutation(UPDATE_PRODUCT_MEDIA_BY_PK, {
    refetchQueries: [{ query: PRODUCTS_BY_PK, variables: { id } }],
  });
  const [deleteProductMedia] = useMutation(DELETE_PRODUCT_MEDIA_BY_PK, {
    refetchQueries: [{ query: PRODUCTS_BY_PK, variables: { id } }],
  });
  ...
  useEffect(() => {
    if (product) {
      ...
      setData({
        ...
        size: product.products_by_pk.size,
        humidity: product.products_by_pk.humidity,
        light: product.products_by_pk.light,
        temperature: product.products_by_pk.temperature,
        occasion:product.products_by_pk.occasion
      });
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        ...
        if (data.main_image_url !== product.products_by_pk.main_image_url) {
          formData.main_image_url = await getFileUrl(
            "products",
            data.main_image_url,
            "image"
          );
    ...
    <>
      ...
      <form className="grid grid-cols-12 gap-2">
        ...
        <FormConstantSelect
          ...
          error={error?.size}
          data={SIZES}
          customFun={(e) => inputHandler(e.target.value, "size")}
        />

        <FormInput
          label="Humidity"
          placeHolder="Enter Plant Humidity Percent "
          value={data.humidity}
          error={error?.humidity}
          customFun={(e) => inputHandler(e.target.value, "humidity")}
        />
        <FormInput
          label="Light"
          ...
          value={data.light}
          error={error?.light}
          customFun={(e) => inputHandler(e.target.value, "light")}
        />
        ...
        <FormInput
          ...
          customFun={(e) => inputHandler(e.target.value, "temperature")}
        />

        <FormInput
          label="Occasion"
          placeHolder="Enter Plant Occasion"
          value={data.occasion}
          error={error?.occasion}
          customFun={(e) => inputHandler(e.target.value, "occasion")}
        />
        ...
        <FormTextArea
          ...
          error={error?.body_html}
          customFun={(e) => inputHandler(e.target.value, "body_html")}
        />
      ...
      <div className="my-5">
        <div className="flex mb-3">
          <p className="text-2xl font-extrabold tracking-tight text-slate-900 mr-5">
            Media
          </p>
        ...
        {showMediaForm && (
          <form className="grid grid-cols-12 gap-2 border-y-2 my-5 py-2">
            <FormTwoUploads
              label1="upload1"
              label2="upload2"
              value1={mediaData.media_url}
              value2={mediaData.video_thumbnail_url}
              error1={mediaError?.media_url}
              error2={mediaError?.video_thumbnail_url}
              customFun1={(e) =>
                mediaInputHandler(e.target.files[0], "media_url")
              }
              customFun2={(e) =>
                mediaInputHandler(e.target.files[0], "video_thumbnail_url")
              }
            />
            ...
            <FormButton
              processing={false}
              cancelHandler={mediaCancelHandler}
              submitHandler={mediaSubmitHandler}
              cancelText="Cancel"
              submitText="Submit"
            />
          </form>
        ...
        <Table tableHeader={mediaHeader} data={mediaDatas}>
          {mediaDatas.map((data, index) => (
            ...
            >
              ...
              <td className="px-6 py-4">{data.media_type}</td>
              <td className="px-6 py-4">
                <TableButton
                  color={"bg-blue-500"}
                  hoverColor={"hover:bg-blue-700"}
                  customFun={() => editMediaHandler(index)}
                >
                ...
                <TableButton
                  ...
                  customFun={() => deleteMediaHandler(data.id)}
                >
